...
const book = [
    ...
    },
    {
        id: 15,
        date:`2009 `,
        title: `Wagner, C., & Okeke, C.I.O  obj-display-none`,
...
const loadbook = () => {
    ...
    book.map((bookData) => {
        ...
        displaybookData += `
        <div class="col-lg-4 col-md-6 col-12 mb-4 mb-lg-0 book-cover">
...
let searchbook =()=>{
    ...
    bookInput.addEventListener('keyup', () =>{
        let filterKey =bookInput.value.toLowerCase();
        let bookData = document.querySelector('.book-container')
        let bookContainer = bookData.querySelectorAll(".book-cover");
        let falsebook = false
        bookContainer.forEach((removebook) => {
            let dataTitle = removebook.querySelector(".title")
           if(dataTitle.innerHTML.toLowerCase().indexOf(filterKey) < 0){
               ...
               removebook.style.display = "initial"
               falsebook = false
